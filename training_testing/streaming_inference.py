#!/usr/bin/env python3
"""
Complete Streaming Inference for Video-Guard Model
Follows the exact training format for streaming video analysis
"""
...
def dynamic_preprocess(image, min_num=1, max_num=12, image_size=448, use_thumbnail=True):
    """Dynamic preprocessing for images (matching training)"""
...
class StreamingVideoAnalyzer:
    """Complete streaming video analyzer following training format"""
...
- <summary>: Shot is complete, provide a complete shot description"""
    ...
    def process_frame(self, frame: Image.Image) -> tuple:
        ...
        # Apply dynamic preprocessing (matching training)
    ...
    def parse_response(self, response: str) -> tuple:
        """Parse model response to extract all labels and action"""
        labels = []
        action = None
        summary = None
        
        # Clean any hallucinated tokens that shouldn't exist
        cleaned = response
        for unwanted in ['<stream>', '<error>', '</error>', '</video>', '<video>', '<frame>', '</frame>']:
        ...
        # Extract action (must be explicit)
        if '<continue>' in cleaned:
            action = 'continue'
        elif '<summary>' in cleaned:
            action = 'summary'
            # Extract summary text after FIRST <summary> tag
            parts = cleaned.split('<summary>')
            if len(parts) > 1:
                # Take text after first <summary>
                summary_text = parts[1]
                # Clean up - remove duplicate <summary> tags and other noise
                summary_text = summary_text.replace('<summary>', ' ')
                # Stop at any other tag
    ...
    def analyze_video_streaming(self, video_path: str, fps_sample: int = 1, max_frames: int = None) -> Dict[str, Any]:
        ...
        for idx, frame_idx in enumerate(tqdm(frame_indices, desc="Processing frames")):
            ...
            if action == 'summary':
                ...
                # Save clip prompt for final response (without streaming prompt, no leading newline)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Streaming Video Analysis")
    ...
    parser.add_argument('--device', type=int, default=5,
                       help='GPU device ID')
    parser.add_argument('--output', type=str, default='streaming_results.json',
